...
\affiliation{
  ...
  \city{Montr\'eal}
  \country{Canada}}
\email{david.vielfaure@polymtl.ca}
...
\keywords{Swarm Robotics, Information Sharing, Robot Safety}
...
% For the nodes to maintain risk awareness, we implement a distributed
% risk belief map using the virtual stigmergy~\cite{pinciroliTuple2016}
% from the Buzz~\cite{pinciroliBuzz2016} programming language. The belief map is constructed by the robots using CRDTs, represented as key-value pairs where the key is the location of the robot and the value is the corresponding risk. To take account of the dynamic nature of risk, we use a Kalman filter to model
% the radiation state of the environment $\bm{r}$. We
% update this state at every time step based on the robot's observation
% vector $\bm{\rho}$. One advantage of using a Kalman filter is that it
% can take into account the different precision of sensors in a
% potentially heterogeneous robot team. Because a robot only observes
% one cell $\rho(\bm{x}_i)$ at a time, an update to the Kalman filter is
% done with an observation vector $\bm{\rho}$ where all dimensions but
% one remain constant with respect to the previous time step.

% Because $\bm{r}$ is a scalar field, it lends itself well to being
% stored in a CRDT at a low cost (the same would be true of a low-dimension vector field). At each time step, the robots store the updated value of
% $\bm{r}(\bm{x}_i)$ in their stigmergy, where the field's input can be
% used as a key (more precisely, a concatenation of $\bm{x}_{i;x}$ and
% $\bm{x}_{i;y}$). This means that the cost of storing the information
% for a given time step is very low, especially as the keys and values consist of only $\mathcal{O}(log|E|)$ characters and one floating-point number respectively.

% The stigmergy does not directly store probabilities and therefore
% cannot be strictly considered as a belief map according to
% \cite{kobayashiSharingExploringInformation2002}. However,
% $r(\bm{x}_i)$ varies monotonically in the same direction as
% $\mathbb{P}(f_i = 1 | S)$, meaning that it can reliably be used to
% formulate a risk-aware storage policy based on the optimization of
...
\multicolumn{1}{c}{\textit{Topology}} & \multicolumn{1}{c}{\textit{Algorithm}} & \textit{\begin{tabular}[c]{@{}c@{}}Transfer speed \\ (hops)\end{tabular}} & \textit{\begin{tabular}[c]{@{}c@{}}Memory used\\  (\%)\end{tabular}} \\ \midrule
\multirow{3}{*}{Grid-like}            & RASS                                   & 11.45                                                                     & 1.35                                                                 \\
                                      & Hop-Count                              & 9.11                                                                      & 0.61                                                                 \\
                                      & Stigmergy                              & N.A.                                                                      & 100.00                                                               \\ \midrule
\multirow{3}{*}{Scale Free}           & RASS                                   & 11.44                                                                     & 1.95                                                                 \\
                                      & Hop-Count                              & 6.85                                                                      & 0.50                                                                 \\
                                      & Stigmergy                              & N.A.                                                                      & 100.00                                                               \\ \midrule
\multirow{3}{*}{Lennard-Jones}        & RASS                                   & 12.51                                                                     & 1.69                                                                 \\
                                      & Hop-Count                              & 7.32                                                                      & 0.51                                                                 \\
                                      & Stigmergy                              & N.A.                                                                      & 100.00                                                               \\ \midrule
...
%\begin{figure}[h]
	%\centering
    %\includegraphics[width=0.95\columnwidth]{figures/lab_graph_left.png}
    %\caption{Topology used in the physical experiments with the edges connecting the %drones.}
%    \label{cogniflyExperiment}
%\vspace{-2mm}
%\end{figure}
